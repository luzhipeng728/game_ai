...
from models import NPC, NPCType, Tier, Faction
import uuid
import time
...
async def update_npc(npc_id: str, npc_update: NPCUpdate, db: Session = Depends(get_db)):
    ...
    for field, value in update_data.items():
        if value is not None:
            # 处理枚举字段
            if field == 'npc_type':
                setattr(db_npc, field, NPCType(value))
            elif field == 'tier':
                setattr(db_npc, field, Tier(value))
            elif field == 'faction':
                setattr(db_npc, field, Faction(value))
            else:
